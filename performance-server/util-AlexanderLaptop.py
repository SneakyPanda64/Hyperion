...
def transcribe(path):
    ...
    dicts = []
    for i in range(0, 3):
        result = model.transcribe(os.path.join(path, f"audio-{i}.wav"))
        for v in result['segments']:
            start = 0
            if len(dicts) > 0:
                start = dicts[len(dicts)-1]["end"]
            if len(dicts)-1 < i:
                dicts.append({
                    "text": v["text"].strip(),
                    "start": start,
                ...
                new_dict = dicts[i]
                new_dict["text"] += v["text"]
                new_dict["end"] = v["end"]
                dicts[i] = new_dict
            print(f"#{i}, start, {start}, end, {v['end'] + start}, text, {v['text']}")
    print(dicts)
